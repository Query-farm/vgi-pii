...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-pii
...
# ATTACH (`${VGI_PII_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run pii_worker.py`) — the extension
...
#                        `uv run --python 3.13 <repo>/pii_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/pii_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_PII_WORKER (the LOCATION) and, for the out-of-band
...
case "$TRANSPORT" in
  subprocess)
    export VGI_PII_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/pii-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/pii-http-server.log"
    ...
    export VGI_PII_WORKER="http://127.0.0.1:$PORT"
    ;;

  unix)
    # Boot the worker bound to an AF_UNIX socket; poll for the socket file.
    SOCK="${TMPDIR:-/tmp}/pii-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/pii-unix-server.log"
    ...
    export VGI_PII_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_PII_WORKER) ..."
OUT_FILE="$(mktemp)"
set +e
